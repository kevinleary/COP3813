...
function SavePostToDB($_db, $_user, $_title, $_text, $_time, $_file_name, $filterStr)
{
	...
	if (!$stmt->bind_param('ssssss', $_user, $_title, $_text, $_time, $_file_name, $filterStr))
	{
...
function getPostcards($_db)
{
    ...
    while($row = $result->fetch_assoc())
    {
        ...
        . '</div><div class="body"><img src="users/' . $row['IMAGE_NAME'] . '" width="300px" style="-webkit-filter:'.$row['IMAGE_FILTER'].'">' . $row['STATUS_TEXT'] . '</div></div>' ;
    }
